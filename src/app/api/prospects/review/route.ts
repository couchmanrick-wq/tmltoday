...
import { reviewProspects } from '@/lib/leafs-prospects';
...
export async function POST(request: NextRequest) {
  ...
  try {
    const result = await reviewProspects();
    return NextResponse.json({ success: true, reviewedAt: new Date().toISOString(), ...result });
  } catch (error) {
    console.error('Prospect review failed', error);
    return NextResponse.json({ error: 'Prospect review failed' }, { status: 500 });
  }
}
